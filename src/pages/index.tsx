    ...
    const App: React.FC = () => {
        ...
        useEffect(() => {
            ...
            if (!('speechSynthesis' in window)) {
                setAiResponse('Your browser does not support Speech Synthesis. Please use Chrome for full functionality.');
                return;
            }

            // Initialize SpeechRecognition with proper type checking
            const SpeechRecognition = window.SpeechRecognition || window.webkitSpeechRecognition;
            if (!SpeechRecognition) {
                setAiResponse('Speech Recognition is not supported in this browser.');
                return;
            }
            ...
            try {
                ...
                if (recognitionRef.current) {
                    ...
                    recognitionRef.current.onresult = (event: SpeechRecognitionEvent) => {
                        try {
                            ...
                            setAiResponse('Error processing your speech. Please try again.');
                        }
                    };
                ...
                setAiResponse('Failed to initialize speech recognition. Please refresh the page.');
            }
        ...
        const toggleListening = () => {
            if (!recognitionRef.current) {
                setAiResponse('Speech recognition not initialized. Please refresh the page.');
                speakText('Speech recognition not initialized. Please refresh the page.');
                return;
            }
                ...
                try {
                    ...
                    if (error instanceof Error) {
                        if (error.message.includes('already-started')) {
                            errorMessage = 'Speech recognition is already active. Please wait a moment and try again.';
            ...
            try {
                const response = await fetch('/api/motivate', {
                    ...
                    body: JSON.stringify({
                        ...
                        userId: session?.user?.email || 'anonymous', // Pass user email from session
                ...
                <div className="relative z-10 bg-white bg-opacity-95 backdrop-blur-md rounded-3xl shadow-2xl p-8 md:p-12 w-full max-w-3xl text-center border-4 border-white border-opacity-40 transform transition-all duration-500 ease-out scale-95 hover:scale-100 mb-auto">
                    ...
                    <div className="bg-gray-100 rounded-xl p-6 mb-8 shadow-inner border border-gray-200 min-h-[120px] flex items-center justify-center relative overflow-hidden">
                        {isLoading ? (
                            ...
                            <p className={`text-gray-700 text-xl italic ${isSpeaking ? 'animate-pulse text-blue-600 font-bold' : 'text-purple-700'}`}>
                                "{displayedAiResponse}
